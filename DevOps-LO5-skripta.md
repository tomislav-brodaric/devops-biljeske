...
**Format svakog zadatka (po PDF-u):** *uoči grešku → ispravi → objasni zašto je bila kriva.*
...
Dopunjavat ćemo kako naiđemo.

- **Host port < 1024 = privilegiran** (80, 443…) → rootless Podman ga **ne smije** vezati (`Permission denied`). Hostovski port uvijek biraj **≥ 1024**.
- **Neuspjeli `run` ostavi kreirani kontejner** koji i dalje drži `--name` → idući `run` s istim imenom pukne na *"name already in use"*. Riješi s `podman rm <ime>` ili zastavicom `--replace`.
...
```
- `podman run` — stvori NOVI kontejner iz slike i pokreni ga
...
- `-d` (`--detach`) — pokreni u **pozadini**; terminal odmah slobodan, ispiše se samo ID kontejnera
...
- `-f` (na `rm`) — `--force`: prvo ugasi kontejner ako radi, pa ga makni

**Dijagnostičke naredbe (također po dijelovima):**
- `podman ps` — popis kontejnera koji **rade**
- `podman ps -a` — `-a` (`--all`) = i **zaustavljeni/izašli** (tu vidiš `Exited (...)`)
...
**Razlaganje naredbe:**
- `podman run` — stvori i pokreni novi kontejner
...
- `podman rm web` = obriši zaustavljeni kontejner (oslobodi ime)
  - ako `rm` kaže da kontejner radi → `podman rm -f web` (`-f` = force: ugasi pa makni)
...
```
- `--replace` = ako kontejner s tim imenom postoji, tiho ga makni i napravi novi
...
### Greška — `-e KEY` (bez `=value`) ≠ `-e KEY=value`
...
- **`&` umjesto `/` u imenu slike** (`docker.io/library&mysql`): `&` je bashu znak za *pokreni u pozadini*, pa se naredba prelomi na `podman run ... docker.io/library` (u pozadini) + zasebnu naredbu `mysql`. Simptomi: `[1] <pid>`, `bash: mysql: command not found`, i pull `docker.io/library/library:latest` → `access ... denied`. Fix: kosa crta `/`, ne `&`.
...
### Greška — `-p` nema smisla uz host networking
...
**Namjera se vidi iz same naredbe:** `-p 8080:80` je naveden → htio si mapiranje porta. Dakle `--network host` je greška/višak, a NE `-p`.

**Glavni fix — makni `--network host`** (vrati se na bridge + `-p`, radna forma iz Zadatka 1):
```bash
...
(Alternativa samo ako baš trebaš host net — što ovdje nije slučaj: makni `-p` i pusti nginx na port ≥ 1024.)
...
### Greška — kontejner nema dugotrajan glavni proces
...
- `Exited (0)` ≠ kvar — to je uredan izlazak. Problem je samo što si htio da radi.
...
- typo `sčeep` umjesto `sleep` (hrvatska tipkovnica, `č`↔`l`) → još jedna retype-zamka; uhvati prije Entera ili `^C`.

---

## Zadatak 5 — `--rm` + `-d` (kontejner nestane prije nego pročitaš log)
...
### Greška — auto-brisanje stigne prije tebe
...
- Dok ti utipkaš `podman logs job`, kontejnera **više nema** → „no such container".
- To je `--rm` + detached zamka: spojiš auto-brisanje s procesom koji završi prebrzo da bi ga stigao pregledati. (Kod dugotrajnog procesa `--rm` je OK — briše tek kad ga ti zaustaviš.)

### Fix — ovisno što želiš

**A) Vidjeti izlaz odmah → makni `-d`** (pokreni u prvom planu):
```bash
podman run --rm --name job docker.io/library/alpine echo hello
```
Ispiše `hello` ravno u terminal, pa se počisti. **(provjereno uživo ✓)**
- bez `-d` proces drži terminal dok ne završi → vidiš stdout direktno

**B) Log mora PREŽIVJETI za kasnije → makni `--rm`**:
```bash
...
`--rm` je vezan uz **prestanak rada** kontejnera, ne uz `run`. Kod procesa koji odmah završi (`echo`, batch posao), kontejner se obriše prije nego dođeš do `logs`. Treba li ti ispis kratkotrajnog posla: ili gledaj u prvom planu (bez `-d`), ili nemoj brisati (izostavi `--rm`), pa očisti ručno.
...
**Razlaganje naredbe:**
- `podman run` — stvori i pokreni kontejner
- `-d` — u pozadini
...
### Greška — 8 MB je premalo; jezgra ubije proces (OOM)
...
- **`Exited (137)` = OOM kill.** Računica: izlazni kod ubijenog procesa = `128 + broj_signala`; SIGKILL = 9 → `128 + 9 = 137`. (Zapamti: **137 = OOM**.)
- Log stane na prvom retku jer je proces ubijen usred dizanja, prije ičega korisnog.

### Fix — digni memoriju (ili makni limit)
...
- doslovna naredba je pala na **`Address already in use`** za 8080 jer ga već drži `app` (nginx) → prije ponovnog `-p` provjeri zauzete portove (`podman ps`).
...
- `podman exec a ping b` — `exec` = pokreni naredbu u VEĆ pokrenutom kontejneru; ovdje: iz `a` pingaj `b` **po imenu** ← tu pukne
...
### Greška — default mreža nema DNS za imena kontejnera
...
### Fix — napravi custom mrežu i spoji oba kontejnera na nju
...
### Greška 1 — relativna putanja `./html` ovisi o tome gdje stojiš
...
- **Fix: koristi APSOLUTNU putanju** — neovisna o tome gdje stojiš:
```bash
...
⚠️ **Zamka u testiranju:** ako prvo pokreneš mount **s** `:Z`, on TRAJNO relabela folder na hostu → kasniji mount bez `:Z` zatekne već ispravnu oznaku i „radi", pa lažno zaključiš da `:Z` ne treba. Zato čisti test treba **nova mapa** + mount **bez** `:Z` PRVI.
...
- `lstat <x>: no such file or directory` na `-v` → putanja ne postoji (često relativna `./` iz krivog foldera). Riješi apsolutnom putanjom.
...
### Greška — prazan indeks paketa; `install` ne zna gdje je nginx
...
### Fix — `apt-get update` PRIJE installa, u ISTOM `RUN`
...
- **`&&`** = pokreni `install` SAMO ako `update` uspije (lančanje uspjeha).
...
- typo u putanji: `cd ~/devops/lo5/10` umjesto `z10` → `No such file`; pazi na prefiks foldera.
...
- `COPY app.py /app/app.py` — kopiraj `app.py` s build contexta u sliku na `/app/app.py`
...
### Greška — shell forma → PID 1 je `sh`, signali se gube
...
Shell forma (`CMD naredba`) ubacuje `/bin/sh -c` između init sustava i tvog procesa. Taj `sh` postaje PID 1 i ne prosljeđuje signale, pa se aplikacija ne gasi čisto (gubi se i propagacija exit koda). Exec forma (`CMD ["…","…"]`) pokreće binar izravno kao PID 1 → ispravno rukovanje signalima. Pravilo: **`CMD`/`ENTRYPOINT` u exec formi** kad god ti treba čisto gašenje.
...
- shell forma IMA jednu prednost: radi shell-proširenja (`$VAR`, `&&`, `|`). Ako ti to treba, koristi je svjesno: `CMD ["sh","-c","python app.py"]` (eksplicitan sh) ili dodaj `--init` da PID 1 bude pravi init koji prosljeđuje signale.
...
- `COPY . /app` — kopiraj **cijeli** build context (sav kod) u sliku na `/app` ← **OVDJE greška: prerano kopira sve**
...
- Slika se gradi **izolirano** i NE vidi tvoj disk. Datoteke s hosta (`app.py`, `package.json`) postoje u slici tek kad ih uneseš.
- `podman build -t ime .` → točka `.` = **build context** (folder spakiran i poslan builderu; „vreća datoteka" koju build smije koristiti).
- `COPY izvor odredište` = izvadi datoteku **iz contexta** i stavi je **u sliku**. Lanac: tvoj folder → (`.`) context → (`COPY`) u sliku.
- Bez `COPY`-a `RUN npm install` nema `package.json` → nema što instalirati. Zato uopće postoji pitanje „što i kojim redom kopirati".

### Greška — `COPY . /app` ruši cache na svaku izmjenu koda
...
- `COPY . /app` u jedan sloj ubaci **sav** kod. Promijeniš li jedan redak bilo koje `.js` datoteke → taj `COPY` sloj se poništi → **i `RUN npm install` ispod njega** → ovisnosti se reinstaliraju, iako se `package.json` nije ni dotaknuo. Spor build.

### Fix — manifest u svoj sloj, instalacija, pa ostatak koda
...
Što Docker sad vidi:
- diraš **kod** → mijenja se samo sloj 3; sloj 1 netaknut → `npm install` (sloj 2) ostaje **cacheiran** ✓ (brz build)
- diraš **`package.json`** → sloj 1 se mijenja → poništava se i sloj 2 (`npm install` se vrti) i sloj 3. I treba — ovisnosti su se stvarno promijenile.
...
- `.containerignore` (ekvivalent `.dockerignore`) dodatno smanji context i izbjegne nepotrebna poništavanja (npr. ne kopiraj `node_modules`, `.git`).
...
### Greška — `ENV PATH=...` ZAMIJENI, ne dodaje
...
### Fix — PROŠIRI PATH (referenciraj stari `$PATH`)
...
(Napomena: ovo NIJE pravilo iz LO1 — u terminalu je PATH već postavljen od shella pa ga ne diramo. Tek kad u Containerfileu SAM postaviš `ENV PATH`, tvoja je odgovornost ne pregaziti ono što je baza složila.)
...
- isti zid vrijedi za bilo koju popisnu varijablu — zaboraviš `$STARO` → zbrisao si je.
- redoslijed u PATH-u određuje prioritet: što je lijevo, traži se prvo (`/app/bin:$PATH` daje prednost tvojim binarima).
...
```
**Pitanje (zad. 15):** mapiraš `-p 8080:8080` ali ništa ne odgovara — koji je nesklad, i što `EXPOSE` zapravo radi?

**Razlaganje Containerfilea:**
- `FROM ubuntu:24.04` — baza Ubuntu 24.04
- `EXPOSE 8080` — **dokumentira** da kontejner „koristi" port 8080 ← **tu je nesklad**
- `CMD ["python3", "-m", "http.server", "3000"]` — exec forma; pokreni Python HTTP server
  - `-m http.server` = pokreni ugrađeni modul `http.server`
  - `3000` = port na kojem server STVARNO sluša

### Greška — dvije stvari

**1) Nesklad portova:** `EXPOSE 8080`, ali server sluša na **3000**. Mapiraš `-p 8080:8080` → host gađa kontejnerski port 8080, a tamo nitko ne sluša (server je na 3000) → prazno. (Isti tip greške kao zad. 1, samo unutar slike.)

**2) Što `EXPOSE` zapravo radi:** **ništa funkcionalno — samo dokumentira.** To je metapodatak u slici koji *označava* namjeravani port. NE otvara port, NE mapira ništa na host. Stvarno objavljivanje radi jedino **`-p` na `podman run`**. Možeš mapirati i port koji nije u `EXPOSE`, i obrnuto — `EXPOSE` bez `-p` ne čini ništa.

### Fix — uskladi portove + stvarno mapiraj
...
- klasična zamka: uskladiš `EXPOSE` ali zaboraviš da to ništa ne mijenja dok ne dodaš `-p`.
...
- `COPY requirements.txt /app/requirements.txt` — kopiraj datoteku u sliku
- `RUN pip install -r ...` — instaliraj pakete iz requirements

### Greška — prerani prelazak na neprivilegiranog korisnika
...
### Fix — privilegirani koraci kao root; `USER` na kraju, tik prije pokretanja
...
CMD ["python", "app.py"]
```
- `WORKDIR /app` — stvori i uđi u `/app` (još kao root → smije)
...
- za root korak usred „ne-root" dijela možeš se privremeno vratiti (`USER root` pa natrag), ali bolje je sve privilegirano odraditi gore.
...
- `CMD ["/src/app"]` — pokreni binar

### Greška — finalna slika nosi cijeli toolchain
...
### Fix — multi-stage: gradi u jednoj fazi, kopiraj samo binar u minimalnu
...
- `COPY --from=build /src/app /app` — prekopiraj **samo binar** iz faze `build` u finalnu sliku
- `CMD ["/app"]` — pokreni taj binar
...
- isti princip vrijedi za Java (build s Maven/JDK → run na JRE/distroless), Node (build → kopiraj `dist/` + `node_modules`), itd.
...
### Fix — traži realnu količinu (ili makni `requests`)
...
- → tako VIDIŠ razmake/tabove koje inače ne primjećuješ (YAML ne dopušta tabove i osjetljiv je na uvlake)
...
- `-l app=badimg` u `describe`/`get` = rad po labeli, ne moraš znati nasumični sufiks Poda.
...
**Zašto `--previous` (`-p`):** kontejner se upravo srušio i restarta, pa „trenutni" log zna biti prazan/nov. `--previous` čita zadnju srušenu instancu — bez njega često uhvatiš novi pokušaj prije ičega.

### Fix — naredba koja ostane živa
...
- `grep -A5 "Last State"` → `-A5` = redak + 5 redaka iza (After), da uhvatiš Reason + Exit Code

### Fix — digni `limits.memory` iznad stvarne potrebe
...
2. **Deployment: `spec.selector` je immutable** → mijenjaš li selector postojećeg Deploymenta, `apply` javi `field is immutable`. Riješi `kubectl delete deploy <ime>` pa ponovni `apply`. (Razlog: selector određuje koje Podove kontrolira; mijenjati ga usred rada bi osirotilo postojeće Podove.)

### Gotče

- `-l app=X` filtrira po labeli — ako je labela kriva (npr. ostala `app: crashy` od kopiranja), `get pods -l app=oomy` vrati „No resources found" iako Pod POSTOJI. Provjeri `get pods` BEZ filtra, ili `cat manifest` da vidiš stvarnu labelu.
...
- typo u imenu Poda (`...868-tsm29` vs `...868d-tsm29`) → `NotFound`; lakše je `-l <labela>` ili kopiraj puno ime iz `get pods`.
...
### Fix — uskladi probu na port gdje app sluša (80)
...
### Fix — uskladi selector Servisa s labelom Poda
...
- `get endpoints <svc>` → `<none>` = mismatch ILI nema ready Podova. Provjeri i labele (`get pods --show-labels`) i readiness.
...
### Greška — Deployment ne smije imati neusklađen selector/template
...
### Fix — uskladi `template.labels` sa `selector.matchLabels`
...
- Deployment `selector` je k tomu i **immutable** nakon stvaranja (zad. 23) — pa ako mijenjaš selector postojećeg, treba `delete` + `apply`.

---

## Zadatak 27 — Pending: `requests` > kapacitet čvora (`Insufficient cpu`)

> Rođak zad. 19, ali drukčiji u dvije stvari: vrtimo **CPU** (ne memoriju) i broj je *uvjerljiv* (`8`), ne apsurdan kao `100Gi` — pa moraš **provjeriti stvarni kapacitet čvora**, ne procijeniti okom. Glavni novi alat: `kubectl describe node`.
...
### Fix — spusti `cpu` da stane u slobodni Allocatable
...
### Fix — dvije varijante (po namjeri Poda)
...
**Varijanta B — posao je STVARNO kratak i to je OK** (jednokratan task): reci k8s-u da ne restartuje.
```yaml
spec:
...
- Events zovu kontejner „failed" i kad je exit 0 — ne vjeruj riječi „failed", provjeri `Exit Code`.
- `restartPolicy` vrijednosti: **`Always`** (default, za servise) / **`OnFailure`** (restart samo na grešku) / **`Never`** (nikad). Za jednokratni posao biraj zadnja dva → Pod završi u `Completed`.
...
> **Zašto postoji:** `describe pod` da događaje **jednog** objekta. Kad ne znaš ni gdje je problem, trebaš **kronološku traku svega** na klasteru. `get events` po defaultu NIJE poredan po vremenu → zato `--sort-by`.
...
- `kubectl run busy` — imperativno stvori **goli Pod** `busy` (bez manifesta)
...
- `sh -c "echo radim; exit 3"` — shell, ispiši, izađi kodom 3 (namjeran „pad")
...
- **sortirano** → uredna kronološka traka (npr. čisti niz `4h22m → 3h44m`), vidiš tok `Scheduled → Pulling → Pulled → Created → Started → (BackOff/Failed)`
...
Events su kratkotrajni zapisi koje generiraju kontroleri/kubelet/scheduler o tome što rade s objektima. `describe <objekt>` filtrira događaje tog objekta; `get events` daje **sve** u namespaceu. Za dijagnozu „nešto se dogodilo a ne znam gdje" želiš ih **kronološki** — `--sort-by=.lastTimestamp` (ili robusnije `.metadata.creationTimestamp` / `kubectl events`). Pamti da istječu (`--event-ttl`) → nisu zamjena za trajni logging.

### Gotče

- **`get events` bez sortiranja = nasumičan redoslijed.** Uvijek dodaj `--sort-by` za tok.
- `--sort-by=.lastTimestamp` može varati (prazan `.lastTimestamp` kod novih događaja) → koristi `.metadata.creationTimestamp` ili `kubectl events`.
...
> **Zašto postoji:** events/`describe` kažu što k8s vidi IZVANA. „App ne nalazi servis/bazu" je često **DNS iznutra Poda** — vidiš ga samo ako uđeš u Pod i pitaš njegov resolver. Alat: `kubectl exec`. **Nastavak na zad. 30** (tamo pogled izvana, ovdje iznutra).
...
```
- `expose deployment web --port=80` — napravi **Servis** `web` koji cilja Deployment → time dobivaš ime `web` za razriješiti
...
- `exec` — pokreni naredbu u **POSTOJEĆEM** kontejneru (≠ `run` koji stvara novi Pod)
...
1. **Search domene imaju VIŠAK naslijeđen s hosta.** Uz 3 klasterske (`default.svc...`, `svc...`, `cluster.local`) pojavile su se i **`dns.podman` i `vua.cloud`** — to su domene iz VM-ovog `/etc/resolv.conf` (`vua.cloud` = Algebra/VUA cloud). Kubelet (zadani `dnsPolicy: ClusterFirst`) **dopiše čvorove search domene iza klasterskih**. Zato vidiš pokušaje `web.dns.podman`, `web.vua.cloud`. → **Pod-ov search list = klasterske domene + domene čvora.**
...
Svaki Pod dobije `/etc/resolv.conf` koji k8s ubrizga: `nameserver` pokazuje na **CoreDNS** (ClusterIP `kube-dns` servisa), a `search` domene omogućuju kratka imena (`web` umjesto punog FQDN-a). Otkrivanje servisa = DNS A-zapis `<svc>.<ns>.svc.cluster.local` → ClusterIP. `kubectl exec -it <pod> -- sh` te ubaci u kontejner da to provjeriš iznutra (resolv.conf + nslookup) — ključno kad „app ne vidi drugi servis": je li DNS kriv, ili je problem dalje (Endpoints/port). Pazi na busybox nslookup bučnost i na search domene naslijeđene s čvora.
...
- ako Pod NEMA shell (distroless) → `exec -- sh` padne → trebaš `kubectl debug` ephemeral (zad. 32).
...
- `debug` — pokreni debug-sesiju; **dodaje ephemeral kontejner u POSTOJEĆI Pod** (bez restarta, bez novog Poda)
- `-it` — interaktivno + TTY
...
- `--target=distroless` — **dijeli namespace s tim kontejnerom** (PID + ostalo) → vidiš procese/datoteke mete
...
- veza: `exec` (zad. 31) za slike SA shellom; `debug` (ovaj) za one BEZ. Oba služe „uđi i pogledaj iznutra".

---

## Zadatak 33 — `run tmp --rm -it` (privremeni Pod za test povezanosti)

> **Zašto postoji:** `nslookup` (zad. 31) potvrdi DNS, ali ne i da nešto **odgovara** na portu. Za end-to-end test digneš **Pod-jednokratku**, spojiš se na servis, pa ga sam obriše. Nula smeća. **Zatvara dijagnostičku skupinu: name→IP (31) → uđi u distroless (32) → stvarna povezanost (33).**
...
- `--rm` — **auto-obriši Pod čim izađeš** (isti `--rm` kao `podman run`, zad. 5) → throwaway
- `-it` — interaktivno + TTY
...
- `--rm` obriše Pod tek na **čist izlaz** iz shella (`exit`); ako prekineš drugačije, Pod zna ostati → `kubectl delete pod tmp`.
...
Čvor je „spreman" dok kubelet redovito javlja zdrav status API serveru. NotReady znači jedno od: kubelet ne radi/ne javlja (heartbeat zastao → `Unknown`), ili javlja problem (`Ready False` s konkretnim Reasonom — CNI/runtime/pressure). Dijagnoza ide odozgo (`get nodes` → `describe node` → `Conditions`/heartbeat) pa, ako treba i ako imaš pristup čvoru, na sam čvor (kubelet log, disk, runtime). Razlika `False` vs `Unknown` ti odmah kaže je li kubelet živ-ali-nezadovoljan ili posve nedostupan.

### Gotče

- **`Ready False`** (kubelet živ, javlja problem) vs **`Ready Unknown`** (kubelet nedostupan, heartbeat zastao) — različit smjer dijagnoze.
- na NOVOM klasteru NotReady je najčešće **CNI još nije primijenjen** (`NetworkReady=false`) — riješi instalacija CNI-ja, ne paljenje vatre.
...
→ `finalizers:null` je **zadnja opcija**, tek kad **znaš** da je kontroler mrtav i cleanup nepotreban (ili ručno odrađen). Kod izmišljenog finalizera nema pravog resursa → bezopasno; u produkciji oprezno.
...
- isto se događa i **namespaceu** zaglavljenom u `Terminating` (čest slučaj) — uzrok su finalizeri resursa unutra; ista logika (riješi/ukloni finalizere).
...
- Events poruka **imenuje ključ I ConfigMap** (`VELICINA` u `default/appconfig`) → odmah znaš što popraviti.
...
```
- spec Poda je cijelo vrijeme valjan; falio je **vanjski podatak** → popuniš ConfigMap → kubelet u idućem pokušaju pokrene kontejner. **Ne treba `delete`+`apply`.**
...
- `kubectl get ... -w` (watch) = gledaj prijelaze stanja uživo; `Ctrl+C` za izlaz.
...
- `--dry-run=server -o yaml` = vidiš objekt sa svim server-defaultima PRIJE snimanja (provjera „što će stvarno biti").
...
> **Metoda, ne pojedinačni kvar.** Kad „aplikacija ne može do servisa", prođeš **redom niz lanac** od 5 karika; svaka isključi jedan sloj. Sinteza: zad. 24 (readiness), 25 (Endpoints), 31 (DNS), 33 (povezanost).
...
- **(3) Endpoints** — **ključ**: prazno (`<none>`) → selector mismatch / Podovi ne-ready (zad. 25); **popunjeno → gledaj NA KOJEM PORTU**. Ovdje `:8080` = trag (Servis šalje na port gdje nitko ne sluša).
...
- **`Connection refused`** (paket stigao do Poda, port zatvoren) vs **`timeout`** (paket nigdje ne stigao, zad. 33) — refused znači da si bliže meti.
...
- **typo u imenu (ULOVLJENO):** `http://show` umjesto `http://shop` → `bad address 'show'` (DNS ne nađe). Paste ne radi → tipfeleri; provjeri ime.
- `--port` (Servis) vs `--target-port` (Pod) kod `expose` — lako zamijeniti; `--target-port` mora pogoditi port aplikacije.
- veza: ovo objedinjuje zad. 24 (readiness → Pod ready), 25 (Endpoints/selector), 31 (DNS), 33 (wget/timeout-refused). Ako trebaš jednu „master" vježbu za mrežu — ovo je ona.

---

## Zadatak 42 — `jsonpath` (vađenje `state` / `lastState` / `restartCount`)

> Vidio si ova polja u `describe` (zad. 22, 29). Sad ih **precizno vadiš** s `-o jsonpath` — za skripte, brze provjere, monitoring, bez skrolanja kroz cijeli `describe`.
...
- možeš miješati doslovni tekst i polja: `'Restarts={...} LastExit={...}{"\n"}'` → `Restarts=5 LastExit=1`
...
**Obranjive teze (možeš braniti svaku):**
...
- veza: lanac dijagnoze (zad. 41) vrijedi i ispod Ingressa/Routea — ako „app nedostupna izvana", provjeri i Ingress/Route pravila I donji lanac (svc/endpoints/port).

---

# LO5 — KOMPLETNO ✓

**Skupina A (1–8, `podman run`)** ✓ · **Skupina B (9–18, Containerfile)** ✓ · **Skupina C (19–43, k8s dijagnostika)** ✓

Sva 43 zadatka odrađena, većina verificirana uživo na VM-u (`vm69-61`, rootless podman + minikube + kubectl).
