...
- `:wq` + Enter — **w**rite + **q**uit = spremi i izađi
- `:q!` + Enter — izađi BEZ spremanja (odustani od svih izmjena)
...
**Ako se sve zapetlja:** `Esc` (izađi iz insert moda) → `:q!` + Enter (odustani) → pokreni `kubectl edit` ponovo.
...
- isti obrazac (`kubectl run tmp --rm -it ... --restart=Never -- <naredba>`) je univerzalni alat: „ubaci se na sekundu u klaster, provjeri nešto, izađi"
...
- `kubectl run tmp` — stvori i pokreni Pod imena `tmp`
...
- `sh -c '...'` — pokreni shell (`sh`) koji s `-c` izvrši naredbu zadanu u navodnicima. Za ovako jednostavnu naredbu nije nužan (može i `-- nc -zv web-svc 80` direktno), ali je dobra navika kad unutarnja naredba treba shell stvari (cijevi, varijable)
- `nc` — netcat, alat za otvaranje TCP/UDP veza
- `-z` — zero-I/O / scan mode: samo provjeri je li port otvoren (spoji se i javi), bez slanja/primanja podataka
...
### Naredba — stvori headless servis
...
- `kubectl expose deployment web` — stvori servis za Deployment `web` (preuzme njegov selektor `app=web`)
...
- `kubectl create namespace test-ns` — stvori novi namespace `test-ns`
- `-n test-ns` — pokreni `tmp` Pod U namespaceu `test-ns` (ne u `default`)
...
- `kubectl apply -f <putanja>` — pročitaj manifest s DISKA i stvori/ažuriraj servis (`-f` = file)
...
- pravilo: Pod restarta a ne znaš zašto → `describe` → `Events` → tu piše `failed liveness probe`
...
- `-i` stdin otvoren + `-t` TTY → živi terminal; `-- sh` = pokreni shell koji ostaje i čeka tipkanje
...
# tvoj wget (drugačiji user-agent + IP)
...
- dijagnostički uvid: ako se pitaš rade li probe, log to pokaže direktno
...
### Zad. 53 — default ponašanje BEZ ijedne probe
...
### Zad. 52 — startup probe budget za ~2-min boot (konkretne brojke)
...
### Zad. 51 — tcpSocket readiness na redis:6379
...
### Zad. 24 — montiraj ConfigMap kao volumen (svaki ključ → datoteka)
...
### Zad. 25 — montiraj JEDAN ključ na točnu putanju (`subPath`)
...
### Zad. 31 — generički Secret iz literala + dokaz base64 (NIJE enkripcija)
...
### Zad. 35 — cijeli Secret kao env varijable (`envFrom` + `secretRef`)
...
### Zad. 26 + 34 — Secret kao volumen (dekodirane datoteke, restriktivne dozvole, trade-off vs env)
...
- `-l` (malo L = long/dozvole) vs `-1` (broj jedan = jedan unos po retku) izgledaju gotovo isto u monospace fontu — ako nisi siguran, razdvoji: `ls -l -L`. Veliko `-L` = dereference simlinka.
...
### Zad. 37 — Secret s više ključeva, montiraj SAMO jedan (`items`)
...
- `~/devops` (tilda + `/`) = tvoj home `/home/student/devops`. `~devops` (BEZ `/`) = home korisnika `devops`; takav ne postoji → shell ostavi doslovno `~devops` kao relativno ime mape → gedit otvori praznu datoteku na krivom mjestu. Pravilo: `~/` UVIJEK s kosom crtom.
...
### Zad. 32 + 33 — Secret iz cert/key datoteka: generic (`--from-file`) vs tls tip
...
| validacija | nema | provjeri da cert/ključ čine par |
...
### Zad. 23 — izlistaj StorageClasse, PV-ove i PVC-ove
...
### Zad. 29 — `readOnly: true` na mountu (pisanje odbijeno)
...
### Zad. 30 — `sizeLimit` na `emptyDir` (prekoračenje → izbacivanje Poda)
...
### Zad. 2 + 36 — image-pull Secret (docker-registry) + `imagePullSecrets`
...
**Gotča:** tip Secreta i drugi kratki ispisi znaju „odscrolati" izvan ekrana nakon više naredbi — provjeri zasebnom `get ... -o jsonpath='{.type}'` naredbom.
...
### Zad. 16 — StatefulSet redis:7 (3 replike) + headless servis (stabilna imena + per-pod DNS)
...
### Zad. 21 — StatefulSet stvara I gasi Podove po redu (kontrast s Deploymentom)
...
### Zad. 27 — skaliranje StatefulSeta = jedan PVC po replici (+ sudbina PVC-a pri brisanju)
...
### Zad. 17 — DaemonSet (jedan Pod po čvoru)
...
### Zad. 18 — Job (odradi jednom i završi)
...
### Zad. 19 — CronJob (po rasporedu; suspend; lista Jobova)
...
### Zad. 20 — ručno pokreni Job iz CronJoba (`--from=cronjob/...`)
