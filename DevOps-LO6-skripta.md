...
- **CRD** (Custom Resource Definition, definicija vlastitog tipa resursa) — način da u Kubernetes dodaš vlastitu vrstu objekta.
...
**Kako radi Docker:** Po zadanim postavkama Docker koristi takozvanu **bridge mrežu** (mrežni most) na jednom računalu. Svaki kontejner dobije privatnu IP adresu, kontejneri međusobno razgovaraju preko prijevoda adresa (NAT), a prema vanjskom svijetu izlažu se **objavom porta** (kažeš da se port unutar kontejnera vidi na portu računala). Da bi kontejneri razgovarali preko više računala, treba poseban sloj zvan **overlay mreža** (mreža preko mreže) ili ručno podešavanje.
...
**Pojmovi:** **StorageClass** (razred pohrane) je predložak koji opisuje vrstu pohrane. **Dinamičko provisioniranje** znači da se trajni disk (PV) stvori automatski čim aplikacija zatraži pohranu (preko PVC-a), bez ručne pripreme.
...
**Pojmovi:** **Manifest** je tekstualna datoteka koja opisuje željeni objekt. **Kontroler** je program koji stalno usklađuje stvarno stanje sa željenim. **Operator** je kombinacija vlastitog tipa objekta (CRD, Custom Resource Definition) i kontrolera koji automatizira upravljanje nekom uslugom. **Day-2 operacije** su poslovi nakon prvog puštanja u rad: održavanje, nadogradnje, sigurnosne kopije, oporavak nakon kvara.

**Kako radi sa samim manifestima:** Sam napišeš opis željenog stanja i ručno ga održavaš i mijenjaš.

**Kako radi s operatorom:** Definiraš vlastiti tip objekta, na primjer „PostgresCluster" (klaster baze Postgres), a kontroler iza njega zna kako tu uslugu postaviti, raditi sigurnosne kopije, preuzeti vodstvo pri kvaru i nadograditi je. Operativno znanje čovjeka tako je upisano u program.
...
**Pojmovi:** **Iskustvo developera** znači koliko je razvojnom programeru lako i brzo doći od koda do pokrenute aplikacije. **kubectl** je naredbeni alat za upravljanje Kubernetesom. **S2I** (Source-to-Image, iz izvornog koda u sliku) je OpenShiftova značajka koja iz izvornog koda sama izgradi sliku i pokrene aplikaciju. **Builder slika** je posebna slika koja zna izgraditi tvoju aplikaciju.

**Kako radi čisti Kubernetes:** Sam pišeš opise (manifeste) i sam gradiš i šalješ slike; sve ide preko naredbenog alata. Fleksibilno, ali u više koraka.

**Kako radi OpenShift S2I i konzola:** Daš izvorni kod i builder sliku, a OpenShift **sam izgradi sliku i pokrene aplikaciju**. Postoji i grafičko sučelje (GUI, grafičko sučelje) za razvojne programere.

**Argumenti za čisti Kubernetes:** puna kontrola, sve je vidljivo, prenosivo na bilo koju platformu, bez vezivanja za određenog proizvođača; razumiješ svaki sloj.
...
**Obranjiv stav:** Čisti Kubernetes je usmjeren na **kontrolu, fleksibilnost i prenosivost**; OpenShift S2I je usmjeren na **brzinu razvoja i niži prag** jer skriva izgradnju slike i pokretanje. Tim koji je nov u kontejnerima ili kojem je važna brzina dobit će više od OpenShifta; tim koji želi kontrolu i prenosivost izabrat će čisti Kubernetes.
...
**Pojam:** OpenShift je Kubernetes s **dodanim** stvarima (vlastitim objektima i alatima). Selidba na obični (vanilla) Kubernetes znači zamijeniti te OpenShiftove posebnosti standardnim ekvivalentima.
...
**Argumenti protiv (cijena):** stvaran trud prevođenja OpenShiftovih objekata; gube se ugrađeni alati (izgradnja iz koda, grafička konzola, ugrađena integracija); moraš ponovno izgraditi ono što je OpenShift davao gotovo; sigurnosno učvršćivanje postaje tvoj posao.
...
**Ispitna kuka:** Put s OpenShifta na Kubernetes je moguć jer je OpenShift Kubernetes s dodacima, ali moraš zamijeniti Route Ingressom, DeploymentConfig Deploymentom, izgradnju iz koda vanjskim alatom i registrom, a OpenShiftova sigurnosna pravila standardnim mehanizmom. Mijenjaš udobnost proizvođača za prenosivost i niži trošak.

---

## 6. Agenti za izgradnju (CI/CD) unutar Kubernetesa u odnosu na namjenska računala (VM)

**Pojmovi:** **CI/CD** (Continuous Integration and Continuous Delivery, kontinuirana integracija i isporuka) je automatsko spajanje, provjera i objava koda. **Agent** ili **runner** je radnik koji obavlja te poslove izgradnje. Primjeri alata su Jenkins, GitLab i Tekton. **VM** (Virtual Machine, virtualni stroj) je softverski simulirano računalo. **Noisy-neighbor** (bučni susjed) je pojava kada jedan zahtjevan posao na dijeljenom računalu uspori druge.
...
**Obranjiv stav:** Agenti u Kubernetesu pobjeđuju u **elastičnom skaliranju, učinkovitosti i svježoj izolaciji po poslu**, ali je kontejnerska izolacija slabija od one virtualnog stroja, a bučni susjed i borba za procesor na dijeljenim računalima stvaran su rizik za teške izgradnje. Zato biraš Kubernetes za promjenjiv, navalni rad izgradnje, a namjenska računala kad trebaš jaku izolaciju, predvidive performanse ili povlaštene izgradnje.
...
**Pojmovi:** **Oblak (cloud)** su tuđa računala koja unajmljuješ preko interneta. **Čvor (node)** je jedno računalo u klasteru. **Upravljana usluga (managed service)** znači da pružatelj oblaka održava dio sustava umjesto tebe.

**Kroz što se Kubernetes povezuje s oblakom:**
- **cloud-controller-manager** (upravitelj povezan s oblakom) — kad zatražiš javno dostupan Servis, on stvori pravi uređaj za raspodjelu prometa u oblaku; povezuje i diskove te identitete (IAM, upravljanje identitetima i pristupom).
- **Cluster Autoscaler** (autoskaler klastera) — **dodaje cijela računala** kad Podovi ne mogu stati (stanje koje smo vidjeli u LO5, zadatak 27, kad pohlepan zahtjev premaši kapacitet), a kad ima viška, prazna računala uklanja.
...
**Pojmovi:** **Učvršćivanje (hardening)** je smanjivanje mogućnosti za napad strožim postavkama. **Permisivno** znači da je po zadanim postavkama dopušteno mnogo (manje sigurno dok sam ne pooštriš). **Regulirane industrije** (na primjer banke, zdravstvo) imaju zakonske zahtjeve za sigurnost.
...
**Zadane postavke običnog Kubernetesa:** popustljive (Pod se može pokrenuti kao administrator osim ako sam ne dodaš mehanizam **PodSecurity** ili druga pravila); učvršćivanje sam slažeš (PodSecurity razine, NetworkPolicy za odvajanje prometa, dodatni alati za pravila, pretraživanje slika).
...
**Pojmovi:** **Krivulja učenja** znači koliko je teško i dugo nešto naučiti. **Apstrakcija** je skrivanje složenosti iza jednostavnijeg sučelja.
...
**Kako OpenShift odmaže:** skrivena složenost znači da ne naučiš što se ispod događa, pa kad nešto pukne ili se seliš, fali ti razumijevanje; vezanost za platformu; OpenShift ima i svoje pojmove (Route, SCC, DeploymentConfig) koje također treba naučiti; troši više resursa.

**Argumenti za obični Kubernetes:** na početku je teži, ali učiš temelje koji vrijede svugdje, pa je znanje prenosivo.

**Obranjiv stav:** Timu koji je nov u kontejnerima i želi brzo isporučiti, OpenShiftova apstrakcija na početku **pomaže** (manje koraka, učvršćeno, grafičko sučelje). Dugoročno može **odmoći** jer skriva temelje, pa pri kvaru ili selidbi nedostaje razumijevanje. Ovisi želiš li brz početak (OpenShift) ili dublje, prenosivo znanje (obični Kubernetes). Treba reći i da OpenShift ima vlastite pojmove, pa nije da s njim nema što učiti.

**Ispitna kuka:** OpenShift skrivanjem složenosti ubrzava početak timu novom u kontejnerima, ali dugoročno može odmoći jer ne naučiš temelje. Obični Kubernetes je teži na početku, ali daje prenosivo znanje. Izbor ovisi o tome treba li ti brzina ili dubina.

---

## 10. Resursni otisak punog Kubernetesa u odnosu na k3s za malu lokalnu postavku — kada je puni Kubernetes pretjeran

**Pojmovi:** **k3s** je lagana inačica Kubernetesa (jedna mala datoteka, troši znatno manje resursa). **Resursni otisak** znači koliko procesora i memorije nešto troši. **Lokalna postavka (on-premise)** znači rad na vlastitoj opremi, a ne u tuđem oblaku.
...
**Obranjiv stav:** Za malu lokalnu postavku k3s je obično bolji jer troši mnogo manje resursa i lakše se postavlja, a daje gotovo sve što treba. Puni Kubernetes je pretjeran kada imaš malo računala, malo resursa, jednostavne potrebe i nemaš velik tim za održavanje. Puni Kubernetes vrijedi kada ti stvarno trebaju njegove napredne mogućnosti i velika skala.
...
**Pojmovi:** **Docker Compose** je alat koji jednom datotekom opisuje aplikaciju od više kontejnera na jednom računalu i diže ih zajedno. **Otpornost (resilience)** je sposobnost oporavka od kvara. **Jedno računalo (single host)** znači da sve radi na jednom poslužitelju.
...
**Obranjiv stav:** Za mali tim s jednostavnom aplikacijom i bez velikih potreba za skaliranjem, Docker Compose na jednom računalu je razuman izbor jer je jednostavan i jeftin. Skok na Kubernetes ima smisla kada ti zatrebaju otpornost (preživjeti kvar računala), skaliranje preko više računala ili samoizlječenje. Glavna napetost je jednostavnost (Compose) nasuprot otpornosti i skalabilnosti (Kubernetes). Kubernetes se ne uvodi zbog njega samog, nego kada ga potrebe traže.
...
**Pojmovi:** **Vezanost za proizvođača (vendor lock-in)** znači da je teško i skupo prijeći s jednog proizvođača na drugog. **Otvoreni kod (open source)** znači da je softver javno dostupan i svatko ga smije koristiti i mijenjati. **CNCF** (Cloud Native Computing Foundation) je neutralna zaklada koja održava Kubernetes i srodne projekte.

**Kubernetes:** otvoreni standard koji održava neutralna zaklada; radi kod svih velikih pružatelja oblaka; nisi vezan za jednog proizvođača; veća dugoročna sloboda i mogućnost selidbe.

**OpenShift:** proizvod jedne tvrtke (Red Hat); dobivaš podršku i integraciju, ali se vežeš za njihove posebnosti (Route, SCC, izgradnju iz koda) i za licencu; teže je otići.

**Obranjiv stav:** Kubernetes kao otvoreni standard daje veću dugoročnu slobodu jer nisi vezan za jednog proizvođača i možeš se seliti. OpenShift te više veže (svojim posebnostima i licencom), ali zauzvrat daje podršku i integrirane alate. Napetost je sloboda i prenosivost (Kubernetes) nasuprot udobnosti i podršci uz vezanost (OpenShift). Treba dodati i da se blaža vezanost može stvoriti i na samom Kubernetesu ako se previše osloniš na posebnosti jednog pružatelja oblaka.
...
**Pojam:** **Integrirana platforma** znači da je sve na jednom mjestu i radi zajedno (izgradnja, sučelje, sigurnost, registar slika), umjesto da sam slažeš dijelove.

**Argumenti za OpenShift (jer pitanje traži da ga braniš):** sve na jednom mjestu i usklađeno; **podrška proizvođača** (netko koga zoveš kad nešto pukne); ugrađeni alati i za razvoj i za održavanje; **učvršćeno po zadanim postavkama**; brži početak jer ne moraš sam sastavljati i osiguravati sustav. Za tvrtku koja izričito želi gotov, podržan paket, to je velika vrijednost.
...
**Pojmovi:** **Dodjela pohrane (provisioniranje)** je priprema i dodjeljivanje diskova. **Posao na virtualnom stroju** je aplikacija koja radi na klasičnom virtualnom računalu, bez Kubernetesa.
...
**Obranjiv stav:** Kod klasičnih virtualnih strojeva pohrana se tipično priprema ručno i veže za određeno računalo. Kubernetes to čini opisno i automatski, pa pohranu tražiš, ona se stvori i prati Pod gdje god on bio. Kubernetesov pristup je fleksibilniji i prikladniji za okolinu gdje se aplikacije sele između računala; klasični pristup je jednostavniji i predvidiviji za stalne, fiksne poslužitelje.

**Ispitna kuka:** Kod virtualnih strojeva pohranu pripremaš ručno i veže se za računalo. Kubernetes je dodjeljuje opisno i automatski, pa prati Pod gdje god bio. Kubernetes je fleksibilniji za pokretne aplikacije, klasični pristup predvidiviji za fiksne poslužitelje.
...
**Pojmovi:** **Operativni teret (operational overhead)** je koliko vremena i truda održavanje oduzima. **Mlada tvrtka (startup)** je tvrtka u ranoj fazi s malo ljudi i novca.
...
**Nijansa:** ako im Kubernetes baš zatreba, neka uzmu **upravljani** Kubernetes (gdje pružatelj oblaka održava upravljački dio), da smanje teret. Ali za prvo, brzo i jeftino, jednostavnije rješenje je bolje.

**Obranjiv stav:** Za mladu tvrtku s dva inženjera koja mora brzo i jeftino isporučiti, preporuka je **jednostavno rješenje**: Docker Compose ili Podman na jednom računalu, ili upravljana platforma, jer je jeftino, traži malo održavanja i brzo vodi do proizvoda. Samostalno održavan Kubernetes bio bi prevelik teret za dvoje ljudi. Ako kasnije zatrebaju Kubernetes, neka uzmu **upravljanu** inačicu da im pružatelj nosi teret. Načelo je uskladiti alat s veličinom tima i potrebama, a ne uvoditi složenost prerano.

**Ispitna kuka:** Dvoje inženjera koji moraju brzo i jeftino isporučiti trebaju jednostavno rješenje (Compose ili Podman na jednom računalu, ili upravljanu platformu), jer samostalni Kubernetes nosi prevelik teret. Ako Kubernetes zatreba, neka bude upravljani.
...
**Pojmovi:** **Pozadinska usluga (daemon)** je program koji stalno radi u pozadini i kroz koji idu svi zahtjevi. **Bez pozadinske usluge (daemonless)** znači da takvog programa nema. **Bez administratorskih ovlasti (rootless)** znači raditi kao obični korisnik, a ne kao najmoćniji korisnik. **Administrator (root)** je korisnik koji smije sve na računalu.
...
## Okvir za odlučivanje — kako odgovoriti na svako pitanje „što bi izabrao i zašto"

> Ovaj okvir rješava cijelu skupinu pitanja koja traže preporuku rješenja: 10, 11, 13, 15, 17, 18, 19, 20, 21, 22 i 23. Umjesto da svako učiš zasebno, naučiš **jedan način razmišljanja** i primijeniš ga na bilo koju formulaciju.

### Korak 1 — iz zadatka izvuci faktore

Kod svakog scenarija pitaj se sljedeće. Odgovori određuju rješenje.
...
6. **Trošak i teret održavanja:** koliko vremena i novca smiju otići na održavanje? Malo znači jednostavno ili upravljano rješenje; ako su spremni uložiti, dolazi u obzir samostalno održavan Kubernetes.
7. **Podrška proizvođača:** žele li nekoga koga zovu kad nešto pukne? Ako da, OpenShift ili upravljani Kubernetes; ako ne, dovoljan je otvoreni Kubernetes ili jednostavno rješenje.

### Korak 2 — ljestvica rješenja od najjednostavnijeg do najsloženijeg

Svako sljedeće rješenje daje više mogućnosti, ali traži više znanja i truda. Biraš najniže koje zadovoljava potrebe iz Koraka 1.
...
- **k3s, to jest lagani Kubernetes** — kada trebaš mogućnosti Kubernetesa, ali na maloj ili lokalnoj postavci s malo resursa.
- **Upravljani Kubernetes** — kada trebaš Kubernetes, ali nemaš tim za održavanje upravljačkog dijela; pružatelj oblaka ga održava umjesto tebe. (Pojam: upravljani znači da tuđa tvrtka održava dio sustava.)
- **Samostalno održavan puni Kubernetes** — velika skala, imaš tim i znanje, želiš punu kontrolu. (Pojam: samostalno održavan znači da sve održavaš sam.)
- **OpenShift** — kada želiš integriranu, podržanu i učvršćenu platformu s ugrađenim alatima; često biraju regulirane industrije i tvrtke koje vrednuju podršku.

### Korak 3 — kako oblikovati odgovor na ispitu

1. Pročitaj scenarij i naglas izvuci faktore (veličina tima, skala, otpornost, sigurnost, trošak).
2. Poveži ih s jednim rješenjem s ljestvice.
3. Obrazloži izbor kroz te faktore i pošteno navedi cijenu ili kompromis druge strane (time pokazuješ da vidiš obje strane).
4. Završi jasnom, jednom rečenicom preporuke.
...
- **17 (upravljani ili samostalni Kubernetes):** presuđuje koliko tima i vremena imaš za održavanje (nadogradnje, skaliranje računala, sigurnosne kopije). Malo znači upravljani.
...
- **20 i 21 (bi li izabrao Kubernetes):** presuđuju orkestracija, otpornost, ekosustav i skalabilnost. Za mikroservise i velike sustave, da.
- **22 i 23 (bi li izabrao OpenShift):** isto kao 20 i 21, ali s naglaskom na integraciju, podršku i učvršćenu sigurnost.

### Četiri stupa koja se stalno spominju (za pitanja 20 do 23)

Pitanja 20 do 23 izričito traže ova četiri svojstva. Evo kratke rečenice po svakom, koju možeš izgovoriti.
...
- **Bogatstvo mogućnosti i sigurnost:** Kubernetes ima širok skup mogućnosti koje sam slažeš. OpenShift dolazi s više toga unaprijed posloženog i učvršćenog.

**Jedno upozorenje za obranu:** nikad ne odgovaraj „uvijek Kubernetes" ili „uvijek OpenShift". Snažan odgovor uvijek glasi „ovisi o sljedećim faktorima", pa navedeš koje, i tek onda zauzmeš jasan stav za taj scenarij. To pokazuje da razumiješ kompromise, a ne da napamet guraš jedno rješenje.
...
**Pojmovi:** **Teret održavanja nakon puštanja u rad (day-2 operacije)** su poslovi koji traju dok sustav radi: nadogradnje klastera, dodavanje i uklanjanje računala, sigurnosne kopije. **Samostalno održavan** znači da sve to radiš sam. **Upravljana usluga** znači da pružatelj oblaka održava dio sustava umjesto tebe.

**Samostalno održavan Kubernetes:** ti sam radiš nadogradnje, skaliranje računala i sigurnosne kopije; imaš pun nadzor, ali to traži velik trud i znanje.

**Upravljani Kubernetes:** pružatelj oblaka održava upravljački dio klastera (nadogradnje i dostupnost), a ti se brineš samo za svoje aplikacije; manje truda, ali manje kontrole i plaćaš uslugu.

**Obranjiv stav:** Samostalno održavan Kubernetes daje punu kontrolu, ali nosi velik teret održavanja (nadogradnje, računala i sigurnosne kopije rade ljudi). Upravljana usluga prebacuje taj teret na pružatelja, pa ti ostaje briga samo za aplikacije, uz cijenu manje kontrole i plaćanja usluge. Za većinu timova bez velikog odjela za održavanje, upravljana usluga smanjuje teret; samostalno održavanje biraš kad trebaš punu kontrolu ili imaš tim za to.

**Ispitna kuka:** Samostalno održavan Kubernetes znači da nadogradnje, računala i sigurnosne kopije radiš sam (puna kontrola, velik trud). Upravljani prebacuje to na pružatelja (manje truda, manje kontrole, plaćaš). Tim bez velikog odjela za održavanje bira upravljani.

---

## 18. Tvrtka za emitiranje medija očekuje nagao i nepredvidiv globalni promet — preporuka rješenja u kontejnerima

**Pojmovi:** **Emitiranje medija (media-streaming)** je usluga koja preko interneta šalje video ili zvuk. **Nagao promet (spiky)** znači da broj korisnika naglo skače i pada. **Globalan** znači da su korisnici po cijelom svijetu.
...
**Pojmovi:** **Docker Swarm** je Dockerov vlastiti, jednostavni sustav za orkestraciju preko više računala. **Prerasti (outgrow)** znači da staro rješenje više ne zadovoljava potrebe.
...
## 20. Bi li izabrao Kubernetes za arhitekturu mikroservisa — naglasak na orkestraciji, otpornosti i ekosustavu

**Pojam:** **Arhitektura mikroservisa** znači da je aplikacija razbijena na mnogo malih, neovisnih servisa koji surađuju (umjesto jedne velike cjeline).
...
**Obranjiv stav:** Da, za arhitekturu mikroservisa izabrao bih Kubernetes jer njegova orkestracija upravlja mnoštvom neovisnih servisa, otpornost ih oporavlja pri kvaru, a ekosustav nudi gotove alate baš za taj način rada. Kompromis je složenost; ako je servisa vrlo malo, jednostavnije rješenje može biti dovoljno. Ali za pravu mikroservisnu arhitekturu prednosti nadmašuju složenost.

**Ispitna kuka:** Da, za mikroservise biram Kubernetes jer njegova orkestracija upravlja mnoštvom servisa, otpornost ih oporavlja, a ekosustav nudi gotove alate za taj način rada. Kompromis je složenost, opravdan kad servisa ima više.

---

## 21. Bi li izabrao Kubernetes za sustave razine velikih tvrtki — naglasak na skalabilnosti, podršci zajednice i bogatstvu mogućnosti

**Pojam:** **Razina velikih tvrtki (enterprise-grade)** znači razinu velikih, ozbiljnih poslovnih sustava (visoka pouzdanost, velika skala, stroge potrebe).
...
**Obranjiv stav:** Da, za sustave razine velikih tvrtki izabrao bih Kubernetes jer skalira na veliko, ima golemu zajednicu (pa i dugoročnu održivost i lako nalaženje stručnjaka) i bogat skup mogućnosti za složene potrebe. Kompromis je da traži znanje i tim za održavanje; za male sustave bio bi pretjeran. Za velike, ozbiljne sustave to je standardni i opravdan izbor.

**Ispitna kuka:** Da, za velike poslovne sustave biram Kubernetes jer skalira na veliko, ima golemu zajednicu (znanje, stručnjaci, održivost) i bogat skup mogućnosti. Kompromis je potreba za znanjem i timom, opravdana na velikoj skali.

---

## 22. Bi li izabrao OpenShift za arhitekturu mikroservisa — naglasak na orkestraciji, otpornosti i ekosustavu
...
**Obranjiv stav:** Da, za mikroservise bih razmotrio OpenShift jer ima istu orkestraciju i otpornost kao Kubernetes (jest Kubernetes ispod), pristup istom ekosustavu, te dodatno integrirane alate i podršku proizvođača, što ubrzava rad tima. Kompromis su trošak licence i vezanost. Biram ga kad je tvrtki važna integracija i podrška; ako su važnije sloboda i niži trošak, biram obični Kubernetes.

**Ispitna kuka:** Da, za mikroservise OpenShift ima istu orkestraciju, otpornost i ekosustav kao Kubernetes (jer to i jest), plus integrirane alate i podršku. Kompromis su trošak i vezanost; biram ga kad su važni integracija i podrška.

---

## 23. Bi li izabrao OpenShift za sustave razine velikih tvrtki — naglasak na skalabilnosti, podršci zajednice i bogatstvu mogućnosti
